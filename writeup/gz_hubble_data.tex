...
%\caption{Spectroscopic redshifts from the ACS-GC, 3DHST, and MUSYC catalogues.
%The number in the upper left of each plot is the percentage of redshifts which
%agree within $\Delta z < 0.05$ between the two catalogues being compared in each
...
goodness-of-fit was evaluated in each bin, for each Task, using a normalised
...
this statistic, the data cannot not be modelled with sufficient confidence for
...
%surface-brightness levels and 7 evolutionary corrections each (the colours
...
however. The fraction of objects identified as stars or artifacts is also much
...
The AEGIS sample had only 485~galaxies matched with GZH, and so the statistical
uncertainties in the analysis are higher. However, at \fbest$>0.8$ there is a
clear separation of featured and unfeatured galaxies, though this is not seen
at lower \fbest.  GOODS galaxies, which are the only comparison sample that
also uses visual morphology, shows a very similar trend, with nearly 100\% pure
samples at the lowest and highest values of \fbest.  

Galaxies from GEMS (which
are only split into two types) also have a strong correlation with \fbest,
which is somewhat surprising (but encouraging) considering the known limits of
assigning galaxy morphology based only on a single \sersic{} parameter. 
...
Galaxies from the three COSMOS morphology catalogues all have morphologies that are
...
%We also emphasise that the previously published COSMOS catalogues are not
...
%detailed comparison of the external catalogues is outside the scope of this
%paper, but we emphasise only that it is \emph{impossible} for any catalogue to be
...
%\end{figure*}
%
%Figure~\ref{fig:comparisons_odd} analyses the same sets of matched galaxies, but
...
%COSMOS catalogues, in particular, early-types, spirals, and irregulars are
...
A detailed comparison of the external catalogues is outside the scope of this
...
pulblications on the entire GZH dataset, rather than the volume-limited sample
...
modelled with functions in discrete redshift and surface brightness bins
...
Table~\ref{tbl:ferengi_bar} summarises the correction types for the
...
are blue, and those with not enough data ($N<5$) are grey. A larger version of
the bin outlined at $z=1.0$ and $20.3 < \mu < 21.0$ $\rm (mag/arcsec^2)$ is
shown in Figure~\ref{fig:f_vs_fbar_zoom}. The mean normalised $\chi^2$ of the
change in vote fraction is 0.08. None of the correlations for higher-order
tasks (including bars) were applied to galaxies in the GZH catalogue.}
\label{fig:f_vs_fbar}
\end{figure*}
...
\caption{Morphological correction categories for the \ferengi{} images summarised in
...
bins (grey regions). Only 17\% (maximum) of \ferengi{} galaxies in the sample
