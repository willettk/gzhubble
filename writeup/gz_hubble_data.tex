...
The votes of individual users who classified galaxies in GZH were combined to
make a vote fraction for each response ($f_{response}$) to a question in the
classification tree. Users' votes were weighted \citep[in a method identical to
that described in][]{wil13} such that users who frequently disagreed with all
other users ended up having very low weights. The user weighting $(w)$ was 1
for the top 95\% of users, as ranked by consistency. For the bottom 5\% of
users, $w$ was designed to drop smoothly and was effectively zero for the
bottom 1\% of classifiers.  Since this only affected a tiny percentage of
the users (and an even smaller percentage of the classifications) the
overall effect on the GZH dataset was minimal. The method was effective,
however, at filtering out contributions from random or deliberately
malicious classifiers.

Classifications each user were aggregated and weighted only if they
were logged into GZH under their username (which was encouraged, but not
required for participation). Classifications by users not logged in were marked
as ``Anonymous''; no weighting was applied to these classifications, but they
were otherwise treated identically in the data processing. 
...
a MySQL backend. Users were shown a randomly-selected color composite image from
the GZH sample; the default showed the image with a black sky background,
although users had the option to invert the color palette if desired. The
...
participation, although users could access an extensive ``Help'' section
...
tree (Figure~\ref{fig:decisiontree}). Every classification began with the user
...
questions in the tree depended on the previous answer(s) given by the user; the
decision tree as designed so that the user answered every question relevant to
the morphology in the process of being identified. Questions that were not
answered were implicitly assumed to be absent in the image --- for example, if
the user identified a galaxy as being smooth, they were not asked to count the
number of spiral arms. For every task, the user chose a single answer before
continuing to the next question; they also had the option to restart any
classification in-progress. 

The GZH decision tree had four broad sets of morphologies classified by the
users. The first option designated stars or image artifacts
(the result of either bad data or incorrect classification by the ACS pipeline
as a galaxy); in this case, the classification process ended and no further questions
were asked of the user. The second set were smooth galaxies, intended to select
ellipticals/early-types; users also measure the relative axial ratio
(roundness) for these galaxies. The third set was for disk/late-type galaxies,
which measured the features necessary to place a galaxy on the standard Hubble
tuning fork (bars, spiral arm, strength of the central bulge). The final set,
which was new in this phase of Galaxy~Zoo and designed for high-redshift targets, asked users to identify
...
galaxy gave users the option of identifying ``odd'' features within the image;
these labels were for relatively rare $(\lesssim1\%)$ phenomena, including dust
