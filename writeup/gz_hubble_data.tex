...
\changes{\subsection{Effects of morphological debiasing}\label{ssec:zeta_results}}
...
\changes{\subsection{Correcting morphologies for classification bias}\label{ssec:zeta}}
...
\noindent \changes{where $f_{\mu,z=0.3}$ is the vote fraction at the lowest redshift in
...
\changes{($-10<\zeta<10$)} and sufficient points to fit each function (\changes{28~original
...
\caption{\changes{Histogram showing the fraction of galaxies that have a finite correction
...
\changes{Figure~\ref{fig:correctable_fraction} shows the
...
$\sim50\%$ of the GZH galaxies. \changes{However, any population studies using
...
strong correlation between \ffeatures{} for both sets of images. \changes{The mean
...
would change morphology, only \changes{0.2\% (9~galaxies)}
