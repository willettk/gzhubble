...
instructions for creating such samples using the vote fractions corresponding
to the tasks shown in Figure~\ref{fig:decisiontree}. More conservative cuts can
be applied to create pure, but not necessarily complete, samples. These are
useful for selecting galaxies exhibiting unique morphologies for individual
case studies.  Looser cuts can be applied to obtain samples with a higher level
of completeness, (but with a potential decrease on the overall purity). We
stress that the details of any selection process will vary based on the
