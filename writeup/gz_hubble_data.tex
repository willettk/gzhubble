...
The intended purpose of the GZH catalog is to provide a simple, yet flexible, means of identifying samples of galaxies with a desired morphological type. Here we provide instructions for creating such samples using the vote fractions corresponding to the tasks shown in Figure~\ref{fig:decisiontree}. We stress that the selection process will vary based on the particular science case. 

To select galaxies of a morphological type identified with a Task offered in the decision tree (see Figure~\ref{fig:decisiontree}, a cut is placed on the vote fraction for that Task ($p_{\rm task}$), as well as the vote fractions for the Tasks preceding it. For example, to select barred galaxies, a cut may be placed on $p_{\rm bar}$ such that only galaxies where a high fraction of votes for this task voted for the $bar,yes$ answer. This is not the only necessary cut, however, since not all users answer this question; only those who have previously selected ``features'' in Task~01, ``not clumpy'' in Task~12, and ``not edge-on'', \emph{or} previously selected ``features'' in Task~01, ``clumpy'' in Task~12, ``$>$ 2 clumps'' in Task~16, and ``spiral arrangement'' in Task~15 will have the opportunity to vote on the bar question, Task~03. To ensure that $p_{\rm bar}$ is well-sampled, cuts on all previous tasks must be applied. 

The flexibility of this catalog allows the user to set their own selection criteria for vote fraction thresholds to create a morphologically-pure sample. In Table~\ref{tbl:thresholds} we offer baseline cuts for selecting galaxies of a variety of morphologies. {\note As an example: to select barred galaxies, one should select objects with $p_{\rm features}>0.52, p_{\rm clumpy,no} > 0.55,$ and $p_{edgeon,no}>x.xx$ \emph{or} objects with $p_{\rm features}>0.52, p_{\rm clumpy,yes} > x.xx, (p_{\rm clump~count, 3}+p_{\rm clump~count,4}+p_{\rm clump~count,>4}+p_{\rm clump~count,can't~tell})>x.xx,$ and $p_{\rm clump~arrangement,spiral}>x.xx.$} These cuts define a sample of galaxies of ``bar candidates''; ie galaxies for which answering the bar question makes physical sense and the vote fraction $p_{\rm bar}$ is well-sampled. In this example, such galaxies are featured, non-clumpy, non-edge~on galaxies, or featured, clumpy, spiral galaxies. At this point a cut can be made on $p_{\rm bar}$ to select a clean sample of only strong bars $p_{\rm bar}>0.8$, or a complete sample of weak and strong bars $p_{\rm bar}>0.3$, or the value most appropriate to the science case. 

{\note
\begin{itemize}
    \item explain decision tree format; vote fractions are dependent on previous task vote fractions
    \item provide example of how to select bars
    \item discuss difference between conservative/clean sample and loose/complete sample, emphasize goals differ depending on science case
\end{itemize}
}
...
\subsection{Comparison to other catalogs}\label{ssec:comparisons}
...
The COSMOS galaxies have several published datasets automatically classifying their morphologies, all using a variation of non-parametric measurements clustered with machine algorithms and trained on a visually-classified reference set of several hundred galaxies. \citet{cas07} use a combination of $C$, $A$, $G$, and $M_{20}$ \citep{cas05} to classify all galaxies with $m_\mathrm{I,petro}<25$. They divide galaxies into three broad morphological categories: early-type, spirals, and irregulars. A very similar method is employed by \citet{tas11}; they use the same non-parametric indices to determine galaxy morphology, but with a slightly different method of calculating the Petrosian radius. 64\% of the galaxies have identical classifications in \citet{cas07} and \citet{tas11}; roughly two-thirds of the disagreements are galaxies originally classified as ``irregular'' in \citet{cas07} but ``spiral'' in \citet{tas11}. 

Using a 0.5\arcsec{} positional match, we find 76,241 galaxies with morphologies in both GZH and \citet{cas07}. At the lowest values of \pbest, $\sim10\%$ of the \citet{cas07} galaxies are classified as irregular, only $\sim30\%$ as early-type, and $60\%$ as spiral. The early-type fraction drops with increasing \pbest, with an increasing proportion of irregulars. The \podd{} vote fraction distinguishes sharply between spirals and irregulars at \podd$>0.2$. 

Despite using a similar classification method to \citet{cas07}, the \citet{tas11} classifications show significantly different trends as a function of GZH classifications. Spiral galaxies consistently make up $\sim60\%$ of the sample at \emph{all} \pbest, with a mild increase at \pbest$\simeq1.0$. Early-type and irregular galaxies make up the remainder, smoothly varying between irregulars at low \pbest{} and early-types and high \pbest. 

% Show the fraction of galaxies in each category as a function of p_features and p_odd?
